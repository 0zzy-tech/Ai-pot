...
KNOWN_PATHS = {
    ...
    # ── vLLM ─────────────────────────────────────────────────────────────────
    "/ping", "/version", "/v1/tokenize", "/v1/detokenize",

    # ── LM Studio (/api/v0/ prefix handled via KNOWN_PATH_PREFIXES) ──────────
}

# ── Scanner user-agents ───────────────────────────────────────────────────────
SCANNER_UA_PATTERNS = re.compile(
    r"nikto|sqlmap|nmap|masscan|zgrab|dirbuster|gobuster|nuclei|"
    r"metasploit|burpsuite|zap|wfuzz|ffuf|hydra|medusa|shodan|censys",
    re.IGNORECASE,
)
...
CREDENTIAL_PATTERNS = [
    re.compile(p, re.IGNORECASE)
    for p in [
        # AWS access key ID (starts with AKIA, ASIA, AROA, AIDA, ANPA…)
        r"A(?:KIA|SIA|ROA|IDA|NPA|NVA|PIA)[0-9A-Z]{16}",
        # AWS secret access key references
        r"aws[_\-]?secret[_\-]?access[_\-]?key",
        r"aws[_\-]?access[_\-]?key[_\-]?id",
        # GCP API key
        r"AIza[0-9A-Za-z\-_]{35}",
        # Azure client secret keyword references
        r"AZURE[_\-]CLIENT[_\-]SECRET",
        r"AZURE[_\-]TENANT[_\-]ID",
        # Generic patterns for secrets being submitted
        r"(?:secret|private)[_\-]?key\s*[:=]\s*['\"]?[A-Za-z0-9+/=_\-]{20,}",
    ]
]

# ── CRITICAL: SSRF attempts ───────────────────────────────────────────────────
SSRF_PATTERNS = [
    re.compile(p, re.IGNORECASE)
    for p in [
        ...
        r"(?:https?://|@)(?:127\.|0\.0\.0\.0|localhost)[:/]",  # loopback in URL
        r"(?:https?://|@)(?:10\.|172\.(?:1[6-9]|2\d|3[01])\.|192\.168\.)",  # RFC1918 in URL
        r"file:///",                               # local file inclusion via URL
        r"gopher://",                              # SSRF via gopher
        r"dict://",                                # SSRF via dict
    ]
]
...
TEMPLATE_INJECTION_PATTERNS = [
    re.compile(p)
    for p in [
        r"\{\{.{0,60}\}\}",          # Jinja2/Twig/Handlebars: {{...}}
        r"\$\{.{0,60}\}",            # JavaScript/Java EL: ${...}
        r"#\{.{0,60}\}",             # Ruby/Thymeleaf: #{...}
        r"<%=.{0,60}%>",             # ERB/ASP: <%= ... %>
        r"\{%.{0,60}%\}",            # Liquid/Jinja block: {% ... %}
        r"<#.{0,60}>",               # FreeMarker: <#...>
        r"\[\[.{0,60}\]\]",          # Thymeleaf inline: [[...]]
    ]
]
...
NOSQL_INJECTION_PATTERNS = [
    re.compile(p, re.IGNORECASE)
    for p in [
        r'"\$where"\s*:',            # MongoDB $where (arbitrary JS)
        r'"\$eval"\s*:',             # MongoDB $eval
        r'"mapReduce"\s*:',          # MongoDB mapReduce
        r'"\$ne"\s*:',               # MongoDB $ne (not-equal bypass)
        r'"\$regex"\s*:',            # MongoDB $regex injection
        r'"\$gt"\s*:\s*""',          # MongoDB $gt empty string bypass
        r'\bdb\.\w+\.\w+\s*\(',      # MongoDB shell-style db.collection.find()
    ]
]

# ── HIGH: GraphQL introspection ───────────────────────────────────────────────
GRAPHQL_PATTERNS = re.compile(
    r"__schema|__type|introspectionQuery|__typename.*__fields",
    re.IGNORECASE,
)
...
_B64_KEYWORDS = re.compile(
    r"eval|exec|import|system|subprocess|passw|shadow|/etc/",
    re.IGNORECASE,
)

def _contains_encoded_payload(text: str) -> bool:
    """Detect base64-encoded strings that decode to CRITICAL keywords."""
    import base64
...
def classify_request(
    ...
    # Canary token reuse — attacker copied our fake API key and re-submitted it
    from app.canary import contains_canary
    if body_text and contains_canary(body_text):
        return "attack", "CRITICAL", ["canary_token_reuse"]
    ...
    for pattern in CREDENTIAL_PATTERNS:
        if pattern.search(body_text):
            flagged.append("aws_credential_exposure" if "A(?:KIA" in pattern.pattern or "aws" in pattern.pattern.lower() else "credential_exposure")

    for pattern in SSRF_PATTERNS:
        if pattern.search(body_text):
            flagged.append("ssrf_attempt")
            break

    for pattern in TEMPLATE_INJECTION_PATTERNS:
        if pattern.search(body_text):
            flagged.append("template_injection")
            break

    for pattern in NOSQL_INJECTION_PATTERNS:
        if pattern.search(body_text):
            flagged.append("nosql_injection")
            break

    if not flagged and body_text and _contains_encoded_payload(body_text):
        flagged.append("b64_encoded_payload")
    ...
    # Credential stuffing: sensitive key names submitted in JSON body
    if body_json and any(k.lower() in {"password", "passwd", "credentials", "secret"} for k in body_json):
        flagged.append("credential_stuffing")
        return category, "HIGH", flagged
